...
(function() {
  var LINE_PREFIX, childProcess, compact, compactMap, flatten, glob, isRegExp, isString, keys, map, watch, wrap, _, _ref,
  ...
  module.exports = function(path, command, options) {
    ...
    if (include != null) {
      if (isRegExp(include)) {
        ...
        notIncluded = function(file) {
          return !(__indexOf.call(glob.sync(include), file) >= 0);
        };
      }
    }
    if (exclude != null) {
      if (isRegExp(exclude)) {
        ...
        excluded = function(file) {
          return __indexOf.call(glob.sync(exclude), file) >= 0;
        };
      }
    }
    args = [command];
